...
def button_click():
    ...
    while flag:
        ...
        match answer:
            case '1':
                add_new_note(file_name, separator_character)
                print()
            case '2':
                show_notes(file_name, separator_character)
                print()
            case '3':
                reserve_name = ''
                reserve_name = edit_note(file_name, separator_character)
                if len(reserve_name) > 0:
                    os.remove(reserve_name)
                print()
            case '4':
                reserve_name = ''
                reserve_name = delete_a_note(separator_character, file_name)
                if len(reserve_name) > 0:
                    os.remove(reserve_name)
                print()
            case '0':
